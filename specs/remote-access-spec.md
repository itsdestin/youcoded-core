...
**Feature location:** `youcoded/desktop/src/main/remote-server.ts`, `youcoded/desktop/src/main/remote-config.ts`, `core/skills/remote-setup/SKILL.md`

## Purpose

Remote access allows users to control YouCoded from any device on their network via a web browser. The system has two parts: a WebSocket-based remote server built into the YouCoded desktop app (added v2.1.5), and a guided setup skill (`/remote-setup`) that configures Tailscale networking and authentication.

## User Mandates

- (2026-03-26) Password hashes must use bcrypt. Plaintext passwords must never be stored in config files.
- (2026-03-26) The remote server must only listen when explicitly enabled via `youcoded-remote.json`. It must not start by default on fresh installs.

## Design Decisions

| Decision | Rationale | Alternatives considered |
|----------|-----------|----------------------|
| WebSocket bridge in Electron main process | Reuses existing SessionManager and HookRelay instances — avoids duplicating session management. IPC surface is small (~15 methods). See `youcoded/desktop/docs/superpowers/specs/2026-03-24-remote-web-access-design.md`. | Separate server process (rejected: shared state coordination overhead with no benefit) |
...
| Port 9900 default | Unlikely to conflict with common services. Configurable via `youcoded-remote.json`. | Dynamic port (rejected: harder to bookmark/remember), port 80/443 (rejected: requires root/admin) |
...
|-----------|----------|---------|
| Remote server | `youcoded/desktop/src/main/remote-server.ts` | HTTP + WebSocket server, auth, event broadcasting |
| Remote config | `youcoded/desktop/src/main/remote-config.ts` | Reads/writes `~/.claude/youcoded-remote.json` |
| Remote shim | `youcoded/desktop/src/renderer/remote-shim.ts` | Replaces Electron IPC with WebSocket transport for browser clients |
| Remote setup skill | `core/skills/remote-setup/SKILL.md` | Guided Tailscale + password configuration |
| Config file | `~/.claude/youcoded-remote.json` | Password hash, port, enabled flag, Tailscale trust |
...
- **Depends on:** YouCoded desktop app (Electron), Tailscale (optional but recommended for secure networking), Node.js (bcrypt hashing)
